...
//查询 用户的数量
$dor_all_num = mysqli_num_rows(mysqli_query($conn,"SELECT * FROM dormitory"));
                    ...
                    <form class="am-form am-form-horizontal" method="post" name="add">
                        ...
                        <script>
                            function request(id) {
                                ...
                                window.location.href = "?r=dor-add" + "&db_name_select=" + data;
...
if ($add != "") {
    ...
    if ($user_power!= 3) {
        if ($stu_dor == "") {
            ...
            $isDorExist = queryIsDorExistByDorNameAndDorBuildId($conn,$stu_dor_build_id, $stu_dor);
            if ($isDorExist) {
                echo "<script>showErrorBox('宿舍已存在！');</script>";
                exit ();
            }
        }
//        mysqli_query($conn,"INSERT INTO dormitory (db_id, d_name, d_stu_num_now, d_bed_num) VALUES ($stu_dor_build_id, '$stu_dor', '0', '4')") or die ('SQL语句有误：' . mysqli_error($conn));
        echo "<script>console.log($dor_all_num + 1)</script>";
        echo "<script>console.log($stu_dor)</script>";
        echo "<script>console.log($stu_dor_build_id)</script>";
        mysqli_query($conn,"INSERT INTO dormitory (Dor_Id, Dor_No ,Dor_BedNum, Dor_Person, Build_Id) VALUES ( $dor_all_num + 1, $stu_dor, 4, 0 ,$stu_dor_build_id)") or die ('SQL语句有误：' . mysqli_error($conn));
        echo "<script>showInfoBox('宿舍  $stu_dor_build $stu_dor 已添加！', function() { location.href='?r=show-dormitories&db_id=$dor_build_select_id';});</script>";
        exit ();
    }
}
?>
