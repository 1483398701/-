...
if ($edit_target == "self") {
    $permission_show = "";
    if ($uno == "" || ($user_power == 2 && $uno != $user_no)) {
    ...
    if ($uno == "" || ($user_power != 1 && $uno != $user_no)) {
        header("Location: ?r=permission-denied");
        exit();
//        echo "<script>history.back()</script>";
    } else {
...
// 查询 当前用户的所有信息
$users_current_query = "SELECT * FROM user WHERE Id='$uno' ORDER BY user_Num DESC";
...
if ($an_save != "") {
    ...
    if ($user_power == 1) {
        ...
        mysqli_query($conn, "UPDATE user SET UserName='$an_name' WHERE Id='$an_no'") or die ('SQL语句有误：' . mysqli_error($conn));;
    ...
    if ($an_password != "" && $an_password_new != "" && $an_password_repeat != "") {
        if ($an_password != queryPasswordByUno($conn,$uno)) {
        ...
        if ($user_power == 2) {
            $passMD5Temp = $an_password_new;
            mysqli_query($conn,"UPDATE user SET Password='$passMD5Temp' WHERE Id='$uno'");
                    ...
                    <form class="am-form am-form-horizontal" method="post" name="edit">
                        ...
                        <div class="am-form-group">
                            ...
                            <div class="am-u-sm-9">
                                <input type="text" id="admin-no-normal" name="admin-no-normal" placeholder="请输入你的管理员号" readonly="true"
                                       value="<?php echo $users_current['Id'] ?>">
                            </div>
                        </div>
                        ...
                        <div class="am-form-group">
                            ...
                            <div class="am-u-sm-9">
                                <input type="text" id="admin-name-normal" name="admin-name-normal" placeholder="请输入你的姓名"
                                       value="<?php echo $users_current['UserName'] ?>" <?php echo $permission_read ?>>
                        ...
                        <div class="am-form-group">
                            <div class="am-u-sm-9 am-u-sm-push-3">
                                ...
                                <input type="submit" name="an_save" value="保存修改" class="am-btn am-btn-primary" <?php if ($edit_target == "others" && $user_power == 1) echo ""; else echo $permission_show ?>>
